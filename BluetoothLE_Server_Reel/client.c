    ...
    switch (state) {
        ...
        case CLIENT_DISCOVERING_CHARACTERISTIC:
            if (hci_event_packet_get_type(packet) == GATT_EVENT_CHARACTERISTIC_QUERY_RESULT) {
                ...
                // Enable indications on the server
                uint8_t status;
                if (ping_characteristic.properties & ATT_PROPERTY_INDICATE) {
                    status = gatt_client_write_client_characteristic_configuration( handle_gatt_client_event, connection_handle, &ping_characteristic, GATT_CLIENT_CHARACTERISTICS_CONFIGURATION_INDICATION);
                } else {
                    status = gatt_client_write_client_characteristic_configuration(handle_gatt_client_event, connection_handle, &ping_characteristic, GATT_CLIENT_CHARACTERISTICS_CONFIGURATION_NOTIFICATION);
                }

                if (status == ERROR_CODE_SUCCESS) {
                    printf("Successfully enabled indications on server!\n");
                } else {
                    printf("Failed to enable indications! Error 0x%02X\n", status);
                }
            }
            break;
